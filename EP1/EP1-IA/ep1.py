...
class SegmentationProblem(util.Problem):
    def __init__(self, query, unigramCost):
        ...
        self.tokens = []
        self.remaining_query = self.query
    ...
    def initialState(self):
        ...
        state = (self.tokens, self.remaining_query) 
        return state
    ...
    def actions(self, state):
        ...
        """
        tokens, remaining_query = state
        choices = []
        for i in range(1, len(remaining_query)+1):
            new_token = remaining_query[:i]
            new_remainingquery = remaining_query[:i]
            action = 
            choices.append()
            ...
            choices.append(,,self.unigramCost(self.remaining_query[i:]))
        return choices
        

    def nextState(self, state, action):
        """ Metodo que implementa funcao de transicao """
        (['cat'], 'sanddog') or (['cats'], 'anddog')
        tokens = 
        remaining_query = 
        return state
...
def segmentWords(query, unigramCost):
    ...
    # BEGIN_YOUR_CODE 
    segmentation = SegmentationProblem()
